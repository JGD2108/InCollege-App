           ...
           *> If valid, add the connection
           IF WS-VALID-REQUEST = "Y"
               ...
               STRING "Connection request sent to " DELIMITED BY SIZE
                      FUNCTION TRIM(LK-RECIPIENT) DELIMITED BY SIZE
                      "." DELIMITED BY SIZE
                 INTO LK-MESSAGE
               END-STRING
               MOVE "Y" TO LK-STATUS
           ...
           PERFORM UNTIL WS-CONN-EOF = "Y"
               READ CONNECTIONS-FILE
                   ...
                   NOT AT END
                       ...
                       MOVE REQUESTER-USERNAME TO WS-CONN-REQUESTER(WS-CONN-COUNT)
                       MOVE RECIPIENT-USERNAME TO WS-CONN-RECIPIENT(WS-CONN-COUNT)
                       MOVE REQUEST-STATUS TO WS-CONN-STATUS(WS-CONN-COUNT)
               END-READ
           END-PERFORM
           ...
           *> Check if already connected, or if duplicate pending request exists
           PERFORM VARYING WS-CONN-IDX FROM 1 BY 1
               ...
               *> Check if already connected (status = 'A')
               IF (WS-CONN-REQUESTER(WS-CONN-IDX) = LK-REQUESTER
                   AND WS-CONN-RECIPIENT(WS-CONN-IDX) = LK-RECIPIENT
                   AND WS-CONN-STATUS(WS-CONN-IDX) = "A")
                 OR (WS-CONN-REQUESTER(WS-CONN-IDX) = LK-RECIPIENT
                   AND WS-CONN-RECIPIENT(WS-CONN-IDX) = LK-REQUESTER
                   AND WS-CONN-STATUS(WS-CONN-IDX) = "A")
               ...
               *> Check if recipient already sent requester a pending request
               IF WS-CONN-REQUESTER(WS-CONN-IDX) = LK-RECIPIENT
                   AND WS-CONN-RECIPIENT(WS-CONN-IDX) = LK-REQUESTER
       ...
       ADD-CONNECTION.
           ADD 1 TO WS-CONN-COUNT
           MOVE LK-REQUESTER TO WS-CONN-REQUESTER(WS-CONN-COUNT)
           MOVE LK-RECIPIENT TO WS-CONN-RECIPIENT(WS-CONN-COUNT)
